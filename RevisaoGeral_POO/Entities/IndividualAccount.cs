...
namespace RevisaoGeral_POO.Entities
{
    class IndividualAccount : Account
    {
        ...
        public IndividualAccount(int number, Client holder, double balance) : base(number, holder, balance)
        {
        }
        ...
        public override void WithDraw(double amount, double withDrawelFee)
        {
            if (amount > Balance)
                ...
                Console.WriteLine("New Balance: $ " + Balance.ToString("F2") + " (Fee: " + withDrawelFee.ToString("F2") + ")");
        }
